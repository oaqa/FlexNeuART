...
CMD="scripts/data/diff_collect.sh -i1 \"$RAW_DIR/FullOct2007.xml.bz2\" -i2 \"$RAW_DIR/manner.xml.bz2\" -o input/ComprMinusManner/ComprMinusManner.gz"
LOG_FILE="log_diff"
run_cmd_complex "$CMD" "$CMD_NAME" "$LOG_FILE"
...
for col in manner compr stackoverflow ; do
  ...
  if [ "$col" == "manner" ] ; then
    ...
    CMD="scripts/exper/test_final_lucene_simple.sh $col 999999"
  fi
  run_cmd_complex "$CMD" "$CMD_NAME" "$LOG_FILE"
done
