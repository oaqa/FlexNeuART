...
@delayed_init
class Worker:
    def __init__(self, args, device_name, model_name,
                       query_dict_all, data_dict, valid_run_head):
        print('Loading model from:', model_name, ' to device', device_name)
        model_holder = ModelSerializer.load_all(model_name)
        self.max_doc_len = model_holder.max_doc_len
        self.max_query_len = model_holder.max_query_len
        if args.override_max_doc_len is not None:
           self.max_doc_len = args.override_max_doc_len
        self.model = model_holder.model
        self.model.to(device_name)
        self.device_name = device_name
        self.query_dict_all = query_dict_all
        self.data_dict = data_dict
        self.valid_run_head = valid_run_head
        self.args = args
    ...
    def __call__(self, query_id):
        query_dict = {query_id: self.query_dict_all[query_id]}
        dataset = query_dict, self.data_dict
        orig_run = {query_id: self.valid_run_head[query_id]}
        return run_model(self.model,
                               device_name=self.device_name,
                               batch_size=self.args.batch_size, amp=self.args.amp,
                               max_query_len=self.max_query_len, max_doc_len=self.max_doc_len,
                               dataset=dataset, orig_run=orig_run,
                               use_progress_bar=False,
                               cand_score_weight=self.args.cand_score_weight,
...
def main(args):
    ...
    for run_dict in pqdm(list(query_dict_all.keys()),
                         [Worker(args=args, device_name=device_name, model_name=fname,
                                 query_dict_all=query_dict_all, data_dict=data_dict, valid_run_head=valid_run_head)
                          for device_name in get_device_name_arr(device_qty)],
