...
class ParadeAvgRanker(BertSplitSlideWindowRanker):
    ...
    def forward(self, query_tok, query_mask, doc_tok, doc_mask):
        ...
        B, N, BS = last_layer_cls_rep.shape

        assert BS == self.BERT_SIZE

        pooled_cls = torch.nn.functional.adaptive_max_pool2d(last_layer_cls_rep,
                                                          output_size=(B, 1, self.BERT_SIZE)).squeeze(dim=1)

        out = self.cls(self.dropout(pooled_cls))
