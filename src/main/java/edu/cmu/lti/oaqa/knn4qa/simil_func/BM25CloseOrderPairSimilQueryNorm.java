...
public class BM25CloseOrderPairSimilQueryNorm extends BM25ClosePairSimilarityQueryNormBase implements QueryDocSimilarityFunc {

  public BM25CloseOrderPairSimilQueryNorm(float k1, float b, 
  ...
  public String getName() {
    return "BM25CloseOrderPair";
  }

  private long makeOrderedKey(int wid1, int wid2) {
  ...
  public float compute(DocEntryParsed query, DocEntryParsed doc) {
    ...
    for (int i = 0; i < query.mWordIdSeq.length - 1; ++i) {
      for (int k = i + 1; k < Math.min(query.mWordIdSeq.length, i + mQueryWindow); ++k) {
        int wid1 = query.mWordIdSeq[i];
        int wid2 = query.mWordIdSeq[k];
        
        if (wid1 >= 0 && wid2 >=0 && wid1 != wid2) {
          long key = makeOrderedKey(wid1, wid2);
